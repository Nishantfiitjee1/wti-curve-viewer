...
# ---------------------------- Data loader ----------------------------
@st.cache_data(show_spinner=True, ttl=0)  # reload each time app restarts
...
def filter_dates(df, range_name):
    today = df["Date"].max()
    if range_name == "Full History":
        return df
    elif range_name == "Last 1 Week":
        start_date = today - timedelta(weeks=1)
    elif range_name == "Last 2 Weeks":
        start_date = today - timedelta(weeks=2)
    elif range_name == "Last 1 Month":
...
normalize = st.sidebar.checkbox("Normalize each curve (z-score)", key="normalize_checkbox")
st.sidebar.header("Export")
do_export = st.sidebar.checkbox("Enable CSV export", key="export_checkbox")
...
with tab1:
    ...
    selected_range_spread = st.selectbox("Select date range to view", options=date_range_options, index=0, key="spread_date_range")
...
with tab2:
    ...
    fly_type = st.radio("Choose Fly type:", ["Manual selection", "Auto month fly"], index=1, key="fly_type_radio")
    
    selected_range_fly = st.selectbox("Select date range to view", options=date_range_options, index=0, key="fly_date_range")
    filtered_df_fly = filter_dates(work_df, selected_range_fly)
    
    selected_flies = []
    if fly_type == "Manual selection":
        num_flies = st.number_input("Number of flies to compare", min_value=1, max_value=5, value=1, step=1, key="num_flies")
        ...
        month_base = st.selectbox("Select base contract for Auto Fly", contracts, key="auto_fly_base")
